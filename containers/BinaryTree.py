...
class BinaryTree():
    ...
    def __init__(self, root=None):
        ...
        cannot insert more than one element in the constructor,
    ...
    def print_tree(self, traversal_type):
        ...
        as a reference on how to write
    ...
    def to_list(self, traversal_type):
        ...
        But by returning the results we can
        also do more computations on the results if needed.
    ...
    def __len__helper(node):
        ...
        if node is not None and node.left:
            counter1 += BinaryTree.__len__helper(node.left)
        if node is not None and node.right:
            counter1 += BinaryTree.__len__helper(node.right)
        return counter1 + 1
