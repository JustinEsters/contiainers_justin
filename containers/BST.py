'''
his file implements the Binary Search Tree data structure.
...
class BST(BinaryTree):
    ...
    def __repr__(self):
        ...
        Using this expression ensures that
        all subclasses of BST will have a correct implementation of __repr__,
    ...
    def _is_bst_satisfied(node):
        ...
        it only checks if the children of
        the current node are less than/greater than,
        rather than ensuring that all
        nodes to the left/right are less than/greater than.
    ...
    def insert(self, value):
        ...
        if value == self.root.value:
            ...
            return self._insert1(value, self.root)

    @staticmethod
    def _insert1(value, node):
        if node.value == value:
            return
        if value < node.value:
            if node.left is None:
                ...
                return BST._insert1(value, node.left)
        else:
            if node.right is None:
                node.right = Node(value)
            else:
                return BST._insert1(value, node.right)
    ...
    def find(self, value):
        ...
        if value == self.root.value:
            ...
            if value < self.root.value:
                return self._find1(value, self.root.left)
            else:
                return self._find1(value, self.root.right)

    @staticmethod
    def _find1(value, node):
        '''
        FIXME:
        Implement this function.
        '''
        ...
        if node.value == value:
            return True
        if value < node.value:
            return BST._find1(value, node.left)
        else:
            return BST._find1(value, node.right)
